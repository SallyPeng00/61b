    ...
    private int makeForwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.successor() == null && sq.direction() != 0) {
                for (Place succPlace : sq.successors()) {
                    Sq succSquare = model.get(succPlace.x, succPlace.y);
                    if (sq.connectable(succSquare)) {

                        nFound += 1;
                        found = succSquare;
                        if (sq.sequenceNum() != 0
                                && succSquare.sequenceNum() != 0) {
                            nFound = 1;
                            break;
                        }
                    }
    ...
    private int makeBackwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                ...
                for (Place predPlace : sq.predecessors()) {
                    Sq predSquare = model.get(predPlace.x, predPlace.y);
                    if (predSquare.connectable(sq)) {
                        nFound += 1;
                        found = predSquare;
                        if (sq.sequenceNum() != 0
                                && predSquare.sequenceNum() != 0) {
                            nFound = 1;
                            break;
                        }
                    }
