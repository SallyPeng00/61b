 ...
 *
 *  @author Josh Hug and Sally Peng
 */
...
public class MatrixUtils {
        ...
        for (int i=0; i<m.length; i++){
            for(int j=0; j< m[0].length; j++){
                ...
                if (i == 0) {
                    ...
                    result[i][j] = Math.min(Math.min(get(result, i-1,j-1), get(result, i-1,j)), get(result,i-1,j+1)) + m[i][j];
                }
            }
        }
     ...
     *
     */
    public static double[][] tr(double[][] e){
        double[][] transpose = new double[e[0].length][e.length];
    ...
    public static double[][] accumulate(double[][] m, Orientation orientation) {
        ...
        {return accumulateVertical(m);}
            ...
            return tr(accumulateVertical(tr(m)));
        }
    }
