...
public class IntList {
    ...
    static IntList dcatenate(IntList A, IntList B) {
        ...
        IntList C = IntList.list(A.head);
        if (A.tail == null) {
            C.tail = B;
        }
        else{
            C.tail = dcatenate(A.tail, B);
        }
        return C;
    }
    ...
    static IntList sublist(IntList L, int start, int len) {
        ...
        IntList LNew = subTail(L, start);
        IntList answer = IntList.list(LNew.head);

        for (int x = len; x > 1; x--) {
            assert answer != null;
            answer = dcatenate(answer, IntList.list(LNew.tail.head));
            LNew = LNew.tail;
        }
        return answer;
     ...
     *  exist, and that START and LEN are >= 0. */
     ...
     private static IntList firstN(IntList A, int n) {
         IntList ans = IntList.list(A.head);
         if (n > 1) {
             ans.tail = firstN(A.tail, n-1);
         }
         return ans;
     }
     static IntList dsublist(IntList L, int start, int len) {
         ...
         IntList LStart = subTail(L, start);
         return firstN(LStart, len);
